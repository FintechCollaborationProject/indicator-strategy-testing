...
class FinanceBacktester:
    ...
    def initialize_indicators(self, indicators_to_use):
        # Filter only numeric columns for indicator calculations
        ...
        for indicator_name in indicators_to_use:
            ...
            if indicator_class:
                if indicator_name == 'EMA':
                    indicators.append(indicator_class(numeric_data, short_window=5, long_window=10))
                elif indicator_name == 'BollingerBands':
                    indicators.append(indicator_class(numeric_data, window=20, num_std_dev=2))
                else:
                    indicators.append(indicator_class(numeric_data))
    ...
    def run_backtest(self):
        if self.data is not None and self.strategy is not None:
            ...
            if 'Date' not in backtest_history.columns:
                if 'index' in backtest_history.columns:
                    backtest_history.rename(columns={'index': 'Date'}, inplace=True)
                else:
                    print("Warning: 'Date' or 'index' column not found in backtest history.")
                    backtest_history['Date'] = pd.to_datetime(self.data['Date'])
    ...
    def print_results(self, all_metrics, annual_returns):
        ...
        print("\nAnnual Returns:")
        print(annual_returns if annual_returns else "No annual returns data available.")
...
def main():
    ...
    indicators_to_use = ['BollingerBands', "RSI", "ADX"]
